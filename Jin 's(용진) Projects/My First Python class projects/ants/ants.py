...
class Insect(object):
    ...
    is_ant = False
    damage = 0
    watersafe = False # add a new attribute to the Insect class named watersafe that is False by default
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    watersafe = True # Since bees can fly, make their watersafe attribute True, overriding the default
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    armor = 1
    min_range = 0 # Naming issues -> changed from minimum_range to min_range 
    max_range = float('inf') # same for max -> naming issues 
    ...
    def nearest_bee(self, hive):
        ...
        while (place.entrance) and (place.entrance is not hive) and (i != self.max_range):
            if place.bees:
                break
            place = place.entrance
            i += 1
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        Place.add_insect(self, insect) # First call Place.add_insect to add the insect, regardless of whether it is watersafe
        # class attribute
        # if the insect is not watersafe, reduce the insect's armor to 0 by invoking reduce_armor
        ...
        A: reduce_armor, in the Insect class
        '''
        # END Problem 3A
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 4A
        "*** REPLACE THIS LINE ***"
        reducing = self.armor - amount 
        if reducing == 0:
            list_of_bee = self.place.bees[:]

            for bee in list_of_bee:
...
class LongThrower(ThrowerAnt):
    ...
    armor = 1
    food_cost = 2
    min_range = 5 # at least 5 places away
    implemented = True   # Change to True to view in the GUI
    # END Problem 4B
...
"*** REPLACE THIS LINE ***"
class WallAnt(Ant):
    """WallAnt does nothing each turn. But has large armor"""
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    armor = 1
    time_to_digest = 3 # NAming issues: time -> time_to_digest
    implemented = True   # Change to True to view in the GUI
    # END Problem 6B
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        if QueenAnt.queenlive: 
            self.impostor = True
        QueenAnt.queenlive = True
        # END Problem 9
    ...
    def action(self, colony):
        ...
        "*** REPLACE THIS LINE ***"
        if self.impostor:
            self.reduce_armor(self.armor) # Reduce 1 damage when got attacked
        else:
            ...
            while place.exit:
                if place.ant:
                    ...
                    if place.ant not in self.twice_damage:
                        place.ant.damage *= 2
                        self.twice_damage.append(place.ant)

                    if (isinstance(place.ant, TankAnt)) or (isinstance(place.ant, BodyguardAnt)):

                        if (place.ant.ant) and (place.ant.ant not in self.twice_damage):
                            place.ant.ant.damage *= 2
                            self.twice_damage.append(place.ant.ant)
                place = place.exit
        # END Problem 9
    ...
    def reduce_armor(self, amount):
        ...
        if (self.armor == 0) and (not self.impostor):
            bees_win() # signal to the simulator that the game is over.
        # END Problem 9
...
'''
Once you've finished this problem,
play a game that includes water.
To access the wet_layout which includes water,
add the --water option (or -w for short) when you start the game.
