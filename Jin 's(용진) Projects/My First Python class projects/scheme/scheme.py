...
def do_if_form(expressions, env):
	...
	if scheme_truep(scheme_eval(expressions.first, env)):
		return scheme_eval(expressions.second.first, env, True) #add
...
def do_and_form(expressions, env):
	...
	# BEGIN PROBLEM 13
	"*** REPLACE THIS LINE ***"
	if expressions is nil: # When nothing
		return True

	if expressions.second is nil: #When the first value only!
		return scheme_eval(expressions.first, env, True)

	value = scheme_eval(expressions.first, env)
	if scheme_falsep(value): # f any of these evaluates to a false value, then False is returned.
		return False
	return do_and_form(expressions.second, env) # it should return the value of the last sub-expression.
	# END PROBLEM 13
...
def do_or_form(expressions, env):
	...
	value = scheme_eval(expressions.first, env)
	if scheme_truep(value): #If any sub-expression evaluates to a true value, return that value
		return value
	return do_or_form(expressions.second, env) #If there are no sub-expressions in an or expression, it evaluates to False
	# END PROBLEM 13
...
# When the true predicate does not have a corresponding result sub-expression, return the predicate value
			if clause.second == nil:
				return test
# When a result sub-expression of a cond case has multiple expressions, evaluate them all and return the value of the last expression.
...
def make_let_frame(bindings, env):
	...
	while bindings:
		check_form(bindings.first, 2, 2) # used to check the structure of each binding
		formals = Pair(bindings.first.first, formals)
		values = Pair(scheme_eval(bindings.first.second.first, env), values)
		bindings = bindings.second
	check_formals(formals) # checks that formal parameters are a Scheme list of symbols for which each symbol is distinct.
